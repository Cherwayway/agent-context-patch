...
without weakening approval, privacy, or recovery guarantees. Feature count and
context size are not success metrics.
...
| --- | --- | --- |
| Approval bypass, privacy exposure, data loss, or failed rollback | One credible case | Stop affected writes and prepare a focused fix immediately. |
...
3. **Commit**: a valid, approved plan cannot be applied safely.
...
fresh task repeats the failure; revert or disable a change when it creates an
approval, privacy, corruption, or recovery regression.
